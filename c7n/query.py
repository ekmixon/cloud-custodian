...
class DescribeSource:
    ...
    def get_permissions(self):
        ...
        if m.permissions_enum:
            ...
            perms = [f'{prefix}:{_napi(m.enum_spec[0])}']
        if m.permissions_augment:
            perms.extend(m.permissions_augment)
        else:
            if getattr(m, 'detail_spec', None):
                perms.append(f"{prefix}:{_napi(m.detail_spec[0])}")
            if getattr(m, 'batch_detail_spec', None):
                perms.append(f"{prefix}:{_napi(m.batch_detail_spec[0])}")
        return perms
...
class ConfigSource:
    ...
    def get_query_params(self, query):
        ...
        s = f"select resourceId, configuration, supplementaryConfiguration where resourceType = '{self.manager.resource_type.config_type}'"
    ...
    def get_listed_resources(self, client):
        ...
        with self.manager.executor_factory(max_workers=2) as w:
            ...
            for resource_set in chunks(resource_ids, 50):
                futures = [w.submit(self.get_resources, resource_set)]
...
class QueryResourceManager(ResourceManager, metaclass=QueryMeta):
    ...
    def get_source(self, source_type):
        ...
        raise KeyError(f"Invalid Source {source_type}")
